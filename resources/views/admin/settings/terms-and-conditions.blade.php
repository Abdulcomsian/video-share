...
@section('content')
    <h4 class="py-3 mb-4">
        <span class="text-muted fw-light">Settings /</span> Terms And Conditions
    </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="card-body">
                    <form action="{{ route('admin:settings.save.content') }}" method="post" id="saveTermsAndConditions"
                        ...
                        <div class="row mb-2">
                            <div class="col-md-12">
                                ...
                                <textarea class="d-none" id="termsAndConditionsContent" name="content" required>{{ old('content', $termsAndConditions->content ?? '') }}</textarea>
                                <input type="hidden" name="slug" value="{{ $termsAndConditionsSlug }}" />
...
@push('my-script')
    ...
    <script>
        ...
        $('#saveTermsAndConditions').parsley()

        var quill = new Quill('#quill-editor', {
            theme: 'snow'
        });
        // Your content in HTML format (retrieved from the backend)
        let savedContent = `{!! old('content', $termsAndConditions->content ?? '') !!}`;
        // Load the saved content into the editor
        quill.clipboard.dangerouslyPasteHTML(savedContent);
        quill.on('text-change', function() {
            document.querySelector('#termsAndConditionsContent').value = quill.root.innerHTML;
        });
